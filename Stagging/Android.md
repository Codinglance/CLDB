{
  ...
  "questions": [
    ...
    },
    {
      ...
      "expectedReadTime": "1 min",
      "markdown": "AndroidManifest.xml is a mandatory configuration file in every Android app that provides essential information to the Android system.\n\n#### Key Points\n- Declares app components (Activities, Services, Broadcast Receivers, Content Providers)\n- Defines required permissions (e.g., Internet, Camera)\n- Specifies app metadata (name, icon, theme, version)\n- Sets SDK requirements (minSdk, targetSdk)\n- Identifies the app’s launch Activity"
    },
    {
      "id": 5,
      "section": "Android Basics",
    ...
    },
    {
      ...
      "expectedReadTime": "1 min",
      "markdown": "Android manages memory automatically to ensure smooth performance and efficient utilization of system resources.\n\n**1. Linux-based Memory Management**  \nAndroid runs on the **Linux kernel**, which handles low-level memory operations such as **paging** and **allocation**.\n\n**2. Garbage Collection (GC)**\nAndroid apps execute on **ART (Android Runtime)**, which automatically frees unused objects through **garbage collection**, so developers do **not** need to manually manage memory.\n\n**3. Process Priority & Lifecycle**\nAndroid assigns **priority levels** to apps based on their state:\n- **Foreground app** – highest priority\n- **Visible app**\n- **Background app**\n- **Cached app** – lowest priority\n\n**4. Low Memory Killer (LMK)**\nWhen system memory is low, Android automatically terminates **low-priority background processes** to free memory.\n\n**5. Shared Resources**\nCommon resources like libraries are **shared across apps** to reduce overall memory consumption.\n\n"
    },
    {
      "id": 13,
      "section": "Android Tools",
    ...
    },
    {
      ...
      "markdown": "#### Difference Between onCreate() and onStart()\n\n#### onCreate()\n- Called **once** when the Activity is first created\n- Used for **initial setup** (UI initialization, variables, binding data)\n- Executes before the Activity becomes visible\n\n#### onStart()\n- Called when the Activity **becomes visible** to the user\n- May be called **multiple times** (e.g., returning from background)\n- Prepares the Activity to enter the foreground\n\n#### In Short\n- **onCreate() → Initialization**\n- **onStart() → Visibility**"
    },
    {
      "id":21,
      "section": "Activity Lifecycle",
    ...
    },
    {
      ...
      "markdown": "#### Activity Lifecycle When Launched Again After Back Press\n\nIf the user presses the **Back button**, the Activity is destroyed.  \nWhen it is launched again, a **new instance** of the Activity is created.\n\nThe lifecycle methods are called in this order:\n\n#### 1. onCreate()\n- Activity is created again\n- UI and data are initialized\n\n#### 2. onStart()\n- Activity becomes visible\n\n#### 3. onResume()\n- Activity gains focus and becomes interactive\n\n#### In Short\nAfter Back Press → Activity destroyed  \nLaunch Again → **onCreate() → onStart() → onResume()**"
    },
    {
      "id": 25,
      "section": "Activity Lifecycle",
      ...
      "markdown": "#### Activity Lifecycle When Home Button Is Pressed\n\nWhen the user presses the **Home button**, the Activity is not destroyed.  \nIt moves to the **background**. The system calls the following lifecycle methods:\n\n#### 1. onPause()\n- Called when the Activity loses focus\n- Pause animations or save temporary data\n\n#### 2. onStop()\n- Called when the Activity is no longer visible\n- Release resources not needed in background\n\n**The Activity remains in memory and can be resumed later.**\n\n#### In Short\nHome Press → **onPause() → onStop()** (Activity goes to background)"
    },
    {
      "id": 26,
      "section": "Activity Lifecycle",
      ...
      "markdown": "#### Activity Lifecycle When App Returns from Background\n\nWhen the app returns from the background (after pressing the **Home button** or switching apps), the existing Activity instance is brought back to the foreground. The system calls the following lifecycle methods:\n\n#### 1. **onRestart()**\n- Called after the Activity was stopped\n- Prepares the Activity to start again\n\n#### 2. **onStart()**\n   - Activity becomes visible to the user\n\n#### 3. **onResume()**\n   - Activity gains focus and becomes interactive\n\n#### In Short\nReturn from Background → **onRestart() → onStart() → onResume()**\n"
    },
    {
      "id": 27,
      "section": "Activity Lifecycle",
    ...
    },
    {
      ...
      "markdown": "#### What Are Launch Modes?\n\nLaunch modes control how an Activity is created and managed in the back stack.\n\n#### Types\n\n- **standard** – Creates a new instance every time (default)\n- **singleTop** – Reuses the Activity if it is already at the top\n- **singleTask** – Only one instance in a task; brings existing one to front\n- **singleInstance** – Only one instance; runs in a separate task\n\n#### In Short\nLaunch modes define **how Activity instances behave in the back stack**.\n"
    },
    {
      "id": 33,
      "section": "Activity Launch Modes",
